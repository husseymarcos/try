...
## CLI Interface
- `trust init [PATH]`: Emits a tiny shell wrapper function for your shell. PATH sets the root (absolute path recommended). The function evals the printed, shell-neutral script to `cd` into selections.
- `trust cd [QUERY]`: Launches the interactive selector. If `QUERY` looks like a Git URL, it performs a clone workflow instead. Prints a shell script to stdout; use via the installed function.
- `trust . [name]`: Shorthand to create a date-prefixed directory and, if inside a Git repo, add a detached worktree. Optional `name` overrides the basename.
- `trust worktree dir [name]`: Same as above but explicit CLI, useful without the shell wrapper.
- `trust clone <git-uri> [name]`: Clones into the root. Default name is `YYYY-MM-DD-user-repo` (strips `.git`). Optional `name` overrides.
...
- `trust .`: Creates a new date-prefixed directory using the current working directory’s basename. If inside a Git repo, a detached worktree is added; otherwise this is a plain directory.
...
- `spec/tests/runner.sh`: Test runner that executes all `test_*.sh` files against any `trust` binary.
- Run tests: `./spec/tests/runner.sh /path/to/trust` (supports wrappers like valgrind).

**Important**: Specs must reflect the full feature set of `trust`. They serve as the canonical reference for behavior, enabling new implementations (in any language) to be validated against the same test suite. When adding or changing features, update both the relevant spec markdown and add corresponding tests.
